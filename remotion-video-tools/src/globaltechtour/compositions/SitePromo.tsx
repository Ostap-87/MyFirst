  ...
  return (
    <AbsoluteFill style={{ backgroundColor: "#070A11" }}>
      ...
      <Sequence durationInFrames={OPEN}>
        ...
        >
          {/* Поле элементов позади знака: белый фон сам по себе пустой, а
              так открытие повторяет фон главной страницы сайта. */}
          <AbsoluteFill>
            <TetraField
              count={120}
              minSize={0.03}
          ...
          >
            <SpinningTetra size={fs(0.86)} degreesPerSecond={72} />
          </div>
          <div
            style={{
              opacity: fogIn,
      ...
      <Sequence from={durationInFrames - CLOSE} durationInFrames={CLOSE}>
        ...
        >
          {/* То же поле, что и на открытии, но с другим зерном: одинаковая
              раскладка в начале и в конце читалась бы как склейка. */}
          <AbsoluteFill>
            <TetraField
              count={105}
              minSize={0.03}
